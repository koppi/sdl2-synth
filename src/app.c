...
int app_init(App *app) {
    ...
    if (!synth_init(&app->synth, 48000, 1024, 64)) return 0;
...
void app_poll_events(App *app) {
    ...
    while (SDL_PollEvent(&e)) {
        ...
        if (e.type == SDL_KEYDOWN && e.key.keysym.sym == SDLK_ESCAPE) app->quit = 1;
...
void app_update(App *app) {
    gui_update(&app->gui);
    char title[256];
    snprintf(title, sizeof(title), "Modular Synth - Voices %d/%d | CPU %.1f%% | Arp: %s",
        synth_active_voices(&app->synth),
        app->synth.max_voices,
        synth_cpu_usage(&app->synth),
