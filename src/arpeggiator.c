...
void arpeggiator_init(Arpeggiator *arp) {
    arp->enabled = 0;
    arp->mode = ARP_UP;
    for (int i = 0; i < ARP_PATTERN_LEN; ++i) arp->pattern[i] = 1;
    arp->step = 0;
    arp->tempo = 120.0f;
    arp->step_phase = 0.0f;
    arp->held_count = 0;
...
void arpeggiator_set_param(Arpeggiator *arp, const char *param, float value) {
    if (!strcmp(param, "enabled")) arp->enabled = (int)value;
    else if (!strcmp(param, "mode")) arp->mode = (ArpMode)((int)value);
    else if (!strcmp(param, "tempo")) arp->tempo = value;
}
...
const char *arpeggiator_mode_str(const Arpeggiator *arp) {
    switch (arp->mode) {
        ...
        case ARP_ORDER: return "ORDER";
        case ARP_RANDOM: return "RANDOM";
        default: return "-";
    }
}
...
int arpeggiator_step(Arpeggiator *arp, float samplerate) {
    if (!arp->enabled || arp->held_count == 0) return -1;
