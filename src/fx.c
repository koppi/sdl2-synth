...
void fx_init(FX *fx, int samplerate) {
    ...
    if (!reverb_state.initialized) {
        int comb_delays[REVERB_COMBS] = {
            (int)(0.0297f * samplerate),
            (int)(0.0371f * samplerate),
            (int)(0.0411f * samplerate),
            (int)(0.0437f * samplerate)
        };
        int allpass_delays[REVERB_ALLPASS] = {
            (int)(0.005f * samplerate),
        ...
        for (int i = 0; i < REVERB_ALLPASS; ++i) {
            allpass_init(&reverb_state.allpassL[i], allpass_delays[i], allpass_feedback);
            allpass_init(&reverb_state.allpassR[i], allpass_delays[i] + 13, allpass_feedback);
        }
...
void fx_set_param(FX *fx, const char *param, float value) {
    if (!strcmp(param, "flanger.depth")) fx->flanger_depth = value;
    else if (!strcmp(param, "flanger.rate")) fx->flanger_rate = value;
    else if (!strcmp(param, "flanger.feedback")) fx->flanger_feedback = value;
    else if (!strcmp(param, "delay.time")) fx->delay_time = value;
    else if (!strcmp(param, "delay.feedback")) fx->delay_feedback = value;
    else if (!strcmp(param, "delay.mix")) fx->delay_mix = value;
    else if (!strcmp(param, "reverb.size")) fx->reverb_size = value;
    else if (!strcmp(param, "reverb.damping")) fx->reverb_damping = value;
    else if (!strcmp(param, "reverb.mix")) fx->reverb_mix = value;
}

void fx_process(FX *fx, float *stereo, int frames) {
    for (int n = 0; n < frames; ++n) {
        float phase = sinf(2.0f * 3.14159265f * fx->flanger_rate * n / fx->samplerate);
        int delay_samp = (int)(fx->flanger_depth * (fx->flanger_bufsize - 1) * (0.5f + 0.5f * phase));
        int pos = (fx->flanger_pos + fx->flanger_bufsize - delay_samp) % fx->flanger_bufsize;
