...
static void gui_get_window_size(SDL_Renderer *r) {
    ...
    SDL_Window *w = SDL_GetWindowFromID(1); // fallback: 1 is usually the first window
    if (w) {
...
void gui_init(Gui *gui, SDL_Renderer *renderer, Synth *synth) {
    ...
    gui_font = TTF_OpenFont("/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf", 14);
    if (!gui_font) {
...
void gui_handle_event(Gui *gui, const SDL_Event *e) {
    ...
    if (e->type == SDL_MOUSEMOTION) {
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            if (knob_hit(pitch_x, base_y + osc_knob_y, pitch_r, mx, my)) { hovered_control = i*4+0; break; }
            if (knob_hit(detune_x, base_y + osc_knob_y, detune_r, mx, my)) { hovered_control = i*4+1; break; }
            if (slider_hit(base_x, base_y + osc_slider_y, osc_w * 0.85f, 20, mx, my)) { hovered_control = i*4+2; break; }
            if (knob_hit(phase_x, base_y + osc_phase_y, phase_r, mx, my)) { hovered_control = i*4+3; break; }
        }
        ...
        int fx_xs[6] = {
            margin_x + fx_gap * 0,
            margin_x + fx_gap * 1,
            margin_x + fx_gap * 2 + 20,
            margin_x + fx_gap * 3 + 20,
            margin_x + fx_gap * 4 + 60,
            margin_x + fx_gap * 5 + 60,
        };
        for (int i = 0; i < 6; ++i) {
            if (knob_hit(fx_xs[i], fx_knob_y, fx_knob_r, mx, my)) { hovered_control = 16+i; break; }
        }

        float mixer_x = window_width * 0.68f;
        for (int i = 0; i < 4; ++i) {
            if (slider_hit(mixer_x, 100+i*50, 100, 20, mx, my)) { hovered_control = 22+i; break; }
        }
        if (slider_hit(mixer_x, 350, 100, 30, mx, my)) hovered_control = 26;

        int arp_knob_x = window_width - margin_x - 60;
        int arp_knob_y = 100;
        if (knob_hit(arp_knob_x, arp_knob_y, 30, mx, my)) hovered_control = 27;
        if (knob_hit(arp_knob_x, arp_knob_y + 60, 10, mx, my)) hovered_control = 28;
    }
    ...
    if (e->type == SDL_MOUSEBUTTONDOWN) {
        ...
        if (hovered_control != -1) {
            ...
            if (hovered_control == 28) {
                arpeggiator_set_param(&gui->synth->arp, "enabled", !gui->synth->arp.enabled);
            }
        } else {
            gui->selected_control = -1;
        ...
        if (mouse_drag_control != -1 && (e->motion.state & SDL_BUTTON_LMASK)) {
            ...
            if (sc >= 0 && sc < 16) {
                ...
                if (v < 0.0f) v = 0.0f;
                if (v > 1.0f) v = 1.0f;
                char pname[16]; snprintf(pname, sizeof(pname), "osc%d", mi+1);
...
void gui_update(Gui *gui) {
    ...
    if (gui->selected_control != -1) {
        ...
        if (anim_state.highlight_phase > 1.0f) anim_state.highlight_phase -= 1.0f;
...
static void draw_knob_border(SDL_Renderer *r, int cx, int cy, int radius, float anim_phase) {
...
static void draw_slider_border(SDL_Renderer *r, int x, int y, int w, int h, float anim_phase) {
    Uint8 cr = (Uint8)(255.0f * (0.7f + 0.3f*sinf(anim_phase*2.0f*M_PI)));
    Uint8 cg = (Uint8)(200.0f + 55.0f * (0.7f + 0.3f*sinf(anim_phase*2.0f*M_PI)));
    Uint8 cb = 50;
    SDL_SetRenderDrawColor(r, cr, cg, cb, 255);
...
static void draw_knob(SDL_Renderer *r, int cx, int cy, int radius, float value, float min, float max, const char *label, int highlight, float anim_phase) {
    SDL_SetRenderDrawColor(r, highlight ? 240 : 200, highlight ? 220 : 200, 80, 255);
...
static void draw_slider(SDL_Renderer *r, int x, int y, int w, int h, float value, float min, float max, int highlight, float anim_phase, const char *label) {
    ...
    SDL_SetRenderDrawColor(r, highlight ? 255 : 200, highlight ? 210 : 200, 50, 255);
...
void gui_draw(Gui *gui) {
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        int phase_r = osc_w * 0.11f;
        int cbase = i*4;
        draw_knob(r, pitch_x, base_y + osc_knob_y, pitch_r, s->osc[i].pitch, -24, 24, "Pitch", gui->selected_control==cbase+0 || hovered_control==cbase+0, pulse);
        draw_knob(r, detune_x, base_y + osc_knob_y, detune_r, s->osc[i].detune, -1, 1, "Detune", gui->selected_control==cbase+1 || hovered_control==cbase+1, pulse);
        draw_slider(r, base_x, base_y + osc_slider_y, osc_w * 0.85f, 20, s->osc[i].gain, 0, 1, gui->selected_control==cbase+2 || hovered_control==cbase+2, pulse, "Gain");
        draw_knob(r, phase_x, base_y + osc_phase_y, phase_r, s->osc[i].phase, 0, 1, "Phase", gui->selected_control==cbase+3 || hovered_control==cbase+3, pulse);
    ...
    draw_knob(r, fx_xs[0], fx_knob_y, fx_knob_r, s->fx.flanger_depth, 0, 1, "FlangerDepth", gui->selected_control==16 || hovered_control==16, pulse);
    draw_knob(r, fx_xs[1], fx_knob_y, fx_knob_r, s->fx.flanger_rate, 0, 5, "FlangerRate", gui->selected_control==17 || hovered_control==17, pulse);
    draw_knob(r, fx_xs[2], fx_knob_y, fx_knob_r, s->fx.delay_time, 0, 1, "DelayTime", gui->selected_control==18 || hovered_control==18, pulse);
    draw_knob(r, fx_xs[3], fx_knob_y, fx_knob_r, s->fx.delay_feedback, 0, 1, "DelayFB", gui->selected_control==19 || hovered_control==19, pulse);
    draw_knob(r, fx_xs[4], fx_knob_y, fx_knob_r, s->fx.reverb_size, 0, 1, "ReverbSize", gui->selected_control==20 || hovered_control==20, pulse);
    draw_knob(r, fx_xs[5], fx_knob_y, fx_knob_r, s->fx.reverb_mix, 0, 1, "ReverbMix", gui->selected_control==21 || hovered_control==21, pulse);

    float mixer_x = window_width * 0.68f;
    for (int i = 0; i < 4; ++i)
        draw_slider(r, mixer_x, 100+i*50, 100, 20, s->mixer.osc_gain[i], 0, 1,
            gui->selected_control==22+i || hovered_control==22+i, pulse, "Mix");
    draw_slider(r, mixer_x, 350, 100, 30, s->mixer.master, 0, 2, gui->selected_control==26 || hovered_control==26, pulse, "Master");
