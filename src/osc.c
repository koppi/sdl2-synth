...
void osc_init(Oscillator *osc, float samplerate) {
  ...
  osc->unison_detune = 0.0f; // No unison spread
  osc->unison_voices = 1;    // Single voice
}
...
float osc_process(Oscillator *osc, float note, float *phase_acc) {
  ...
  for (int voice = 0; voice < osc->unison_voices; ++voice) {
    ...
    *phase_acc += freq / osc->samplerate;
    ...
    switch (osc->waveform) {
    case OSC_SINE:
      voice_output = sinf(2.0f * 3.1415926535f * p);
      break;
    case OSC_SAW:
    ...
    case OSC_TRI:
      ...
      break;
    case OSC_NOISE:
      voice_output = (float)rand() / RAND_MAX * 2.0f - 1.0f; // White noise
      break;
    default:
      voice_output = 0.0f;
    }
