...
void osc_init(Oscillator *osc, float samplerate) {
  ...
  osc->phase = 0.0f;
  osc->detune = 0.0f;
  osc->gain = 0.25f;  // Reduced from 1.0f to 0.25f
...
float osc_process(Oscillator *osc, float note, float *phase_acc) {
  float freq =
      440.0f * powf(2.0f, (note + osc->pitch + osc->detune - 69.0f) / 12.0f);
  *phase_acc += freq / osc->samplerate;
  ...
  switch (osc->waveform) {
  case OSC_SINE:
    return osc->gain * sinf(2.0f * 3.1415926535f * p);
  case OSC_SAW:
    return osc->gain * (2.0f * p - 1.0f);
  case OSC_SQUARE:
    return osc->gain * (p < 0.5f ? 1.0f : -1.0f);
  case OSC_TRI:
    return osc->gain * (4.0f * fabsf(p - 0.5f) - 1.0f);
  default:
    return 0.0f;
  }
}