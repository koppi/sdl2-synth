...
// Simple draw_text for axis labels (uses SDL_ttf, ensure a font is loaded in
// your GUI)
static void draw_text(SDL_Renderer *renderer, const char *text, int x, int y,
                      SDL_Color color, TTF_Font *font) {
  if (!font)
...
void oscilloscope_draw(SDL_Renderer *renderer, const Synth *synth, int x, int y,
  ...
  SDL_RenderDrawLine(renderer, x, y + waveform_height / 2, x + w,
                     y + waveform_height / 2);
  ...
  for (int px = 0; px < w - 1; ++px) {
    ...
    if (i2 >= FFT_SIZE)
      break;
    int y1 =
        y + waveform_height / 2 - (int)(tmp[i1] * (waveform_height / 2 - 4));
    int y2 =
        y + waveform_height / 2 - (int)(tmp[i2] * (waveform_height / 2 - 4));
  ...
  SDL_SetRenderDrawColor(renderer, 80, 80, 120, 255);
  SDL_RenderDrawLine(renderer, spec_area.x, spec_area.y + spec_area.h - 1,
                     spec_area.x + spec_area.w, spec_area.y + spec_area.h - 1);

  // Y axis (vertical)
  SDL_RenderDrawLine(renderer, spec_area.x, spec_area.y, spec_area.x,
                     spec_area.y + spec_area.h);
  ...
  for (int px = 0; px < bins; ++px) {
    ...
    SDL_RenderDrawLine(renderer, spec_area.x + px,
                       spec_area.y + spec_area.h - 1, spec_area.x + px, sy);
  }

  // Axis labels for spectrum
  draw_text(renderer, "0Hz", spec_area.x + 3, spec_area.y + spec_area.h - 18,
            label_col, font);
  draw_text(renderer, "Nyquist", spec_area.x + w - 60,
            spec_area.y + spec_area.h - 18, label_col, font);
  draw_text(renderer, "1.0", spec_area.x + 4, spec_area.y + 4, label_col, font);
  draw_text(renderer, "0", spec_area.x + 4, spec_area.y + spec_area.h - 18,
            label_col, font);
}
